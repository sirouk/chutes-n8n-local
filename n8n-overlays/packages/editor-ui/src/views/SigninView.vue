...
import { MFA_AUTHENTICATION_REQUIRED_ERROR_CODE, MFA_FORM, VIEWS } from '@/app/constants';
...
const passwordLoginToggleLabel = 'Login using other credentials';
const ldapLoginLabel = computed(() => ssoStore.ldapLoginLabel);
const isLdapLoginEnabled = computed(() => ssoStore.isLdapLoginEnabled);
const passwordLoginVisible = computed(
	() => !ssoStore.isChutesLoginEnabled || showPasswordLogin.value,
);
...
<template>
	<div>
		<AuthView
			...
			:with-sso="ssoStore.showSsoLoginButton"
			:show-password-login="passwordLoginVisible"
			:password-login-toggle-label="passwordLoginToggleLabel"
			data-test-id="signin-form"
			@toggle-password-login="showPasswordLogin = true"
